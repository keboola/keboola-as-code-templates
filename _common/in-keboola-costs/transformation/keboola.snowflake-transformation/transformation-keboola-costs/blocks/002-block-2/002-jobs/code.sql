...
SELECT "kbc_job_id"                                          AS "job_id",
       ...
       concat("kbc_component_configuration_id",'_',split_part("kbc_branch_id",'_',1)) AS "component_configuration_branch_id",
       "kbc_component_configuration_id"                      AS "component_configuration_id",
       replace(CONVERT_TELEMETRY_TIMESTAMP("job_start_at"),' Z','')   AS "job_start",
       "job_start"::DATE                                     AS "job_start_date",
       replace(CONVERT_TELEMETRY_TIMESTAMP("job_created_at"),' Z','') AS "job_created",
       "job_created"::DATE                                   AS "job_created_date",
       "job_run_time_sec"                                    AS "job_runtime",
       replace(TO_CHAR(CONVERT_TIMEZONE('UTC', DATEADD(SECONDS, "job_run_time_sec", CONVERT_TELEMETRY_TIMESTAMP("job_start_at"))),'YYYY-MM-DD HH24:MI:SS TZH:TZM'),' Z','') AS "job_end",
...
SELECT MD5(
       ...
       replace(CONVERT_TELEMETRY_TIMESTAMP("snowflake_job_start_at"),' Z','') AS "snowflake_job_created",

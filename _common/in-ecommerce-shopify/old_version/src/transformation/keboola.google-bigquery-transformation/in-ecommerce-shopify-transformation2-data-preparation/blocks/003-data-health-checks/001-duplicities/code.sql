...
SELECT
  'sklad' AS DATA_SOURCE,
  'inventory_items' AS ENTITY,
  'duplicities' AS ERROR_TYPE,
  'Duplicated product:' || P.`title` || ' - ' || CASE
    WHEN NOT PV.`option1` IN ('S', 'XS', 'M', 'L', 'XL', 'XXL', 'XXXL')
    THEN PV.`option1`
    ELSE PV.`option2`
  END || ' / ' || CASE
    WHEN PV.`option1` IN ('S', 'XS', 'M', 'L', 'XL', 'XXL', 'XXXL')
    THEN PV.`option1`
    ELSE PV.`option2`
  END || ' found in sklad.' AS DESCRIPTION

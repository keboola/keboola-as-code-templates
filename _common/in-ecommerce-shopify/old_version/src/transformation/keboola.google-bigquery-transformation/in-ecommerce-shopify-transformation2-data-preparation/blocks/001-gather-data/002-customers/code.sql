...
SELECT
  ...
  SUM(ORDER_TOTAL_PRICE_WITHOUT_WAT) OVER (PARTITION BY ORDER_CUSTOMER_EMAIL ORDER BY CAST(ORDER_DATE AS DATE) ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS ORDER_PRICE_ROLL,
  CASE

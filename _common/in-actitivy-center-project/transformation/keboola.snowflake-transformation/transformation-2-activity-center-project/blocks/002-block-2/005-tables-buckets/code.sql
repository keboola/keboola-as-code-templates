...
UPDATE "md_tables"
SET "last_import_date" = "created"
WHERE "last_import_date" = '';
...
CREATE OR REPLACE TABLE "md_tables" AS
WITH descriptions AS (
    SELECT DISTINCT "table_id", "region", "project_id"
    FROM "md_tables_metadata"
    WHERE TRUE
      AND "key" = 'KBC.description'
      AND "value" IS NOT NULL
)
SELECT mdt.*,
       IFF(d."table_id" IS NULL, FALSE, TRUE) AS "table_has_description"
FROM "md_tables" mdt
     LEFT JOIN descriptions d
               ON TRUE
                   AND d."table_id" = mdt."id"
                   AND d."region" = mdt."region"
                   AND d."project_id" = mdt."project_id";
...
SELECT DISTINCT t."table_project_id",
                ...
                t."table_is_alias",
                COALESCE(CONVERT_KBC_TIMESTAMP(mdt."created"),
                         CONVERT_TELEMETRY_TIMESTAMP(t."table_created"))         AS "table_created",
                COALESCE(CONVERT_KBC_TIMESTAMP(mdt."last_import_date"),
                         CONVERT_TELEMETRY_TIMESTAMP(t."table_last_import"))     AS "table_last_import",
                IFF(mdt."id" IS NULL, TRUE, FALSE)                               AS "table_is_deleted",
                mdt."table_has_description",
...
FROM "tmp_tables" t
     LEFT JOIN "md_tables" mdt
               ON TRUE
                   AND mdt."id" = t."table_id"
                   --AND mdt."project_id" || '-' || IFF(mdt."region" = 'us-east-1', 'US', 'EU') = t."project_id"
WHERE TRUE
  AND t.rn = 1;
...
SELECT DISTINCT b."bucket_project_id",
                ...
                IFF(mdt."id" IS NULL, TRUE, FALSE)                 AS "is_deleted",
                CONCAT(REGION_URL(b."project_id", -1), SPLIT_PART(b."project_id", '-', 1),
                       '/storage-explorer/', b."bucket_id")        AS "bucket_url"
FROM "tmp_buckets" b
     LEFT JOIN "md_tables" mdt
               ON TRUE
                   AND mdt."bucket_id" = b."bucket_id"
                   AND CONCAT_WS('-', mdt."project_id", IFF("region" = 'us-east-1', 'US', 'EU')) = b."project_id"
WHERE TRUE
  AND b.rn = 1;

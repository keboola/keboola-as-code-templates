...
SELECT
  ...
  CAST(NULLIF(LEFT(d.add_time, 10), '') AS DATE) AS date_created,
	CAST(NULLIF(LEFT(d.close_time, 10), '') AS DATE) AS date_closed,
  ...
  '' AS `opportunity_type`,
  CAST(IF(`d`.`deal_value` = '', '0.0', `d`.`deal_value`) AS FLOAT64) AS `opportunity_value`,
  `d`.`deal_currency` AS `currency`,
  '' AS `lead_source`,
  CAST(`s`.`deal_probability` AS FLOAT64) AS `probability`
FROM `deals` AS `d`

...
CREATE OR REPLACE TABLE `order_customer_categories` AS
WITH `first_last_order` AS
(SELECT ORDER_ID,
	ORDER_DATE,
	ORDER_CUSTOMER_EMAIL,
	MIN(CAST(TIMESTAMP(O.ORDER_DATE) AS DATE)) OVER (PARTITION BY O.ORDER_CUSTOMER_EMAIL) AS FIRST_ORDER_DATE,
  	MAX(CAST(TIMESTAMP(O.ORDER_DATE) AS DATE)) OVER (PARTITION BY O.ORDER_CUSTOMER_EMAIL) AS LAST_ORDER_DATE,
  	DATE_DIFF(
   	 CAST(TIMESTAMP(O.ORDER_DATE) AS DATE),
   	 CAST(LAG(TIMESTAMP(O.ORDER_DATE)) OVER (PARTITION BY O.ORDER_CUSTOMER_EMAIL ORDER BY CAST(TIMESTAMP(O.ORDER_DATE) AS DATE)) AS DATE),
    	DAY
  	) AS DIFF,
    (SELECT
      COUNT(1)
    ...
    WHERE
     IO.ORDER_CUSTOMER_EMAIL = O.ORDER_CUSTOMER_EMAIL)
  	 AS ORDER_COUNT,
  	COUNT(*) OVER (PARTITION BY ORDER_CUSTOMER_EMAIL ORDER BY CAST(TIMESTAMP(ORDER_DATE) AS DATE) ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS ORDER_COUNT_ROLL,
	SUM(ORDER_TOTAL_PRICE_WITHOUT_TAXES) OVER (PARTITION BY ORDER_CUSTOMER_EMAIL ORDER BY CAST(TIMESTAMP(ORDER_DATE) AS DATE) ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS ORDER_PRICE_ROLL,
...
ORDER BY
  ...
  CAST(TIMESTAMP(ORDER_DATE) AS DATE) NULLS LAST
)
SELECT
  O.ORDER_ID,
  FIRST_ORDER_DATE,
  LAST_ORDER_DATE,
  DATE_DIFF(CAST(TIMESTAMP(O.ORDER_DATE) AS DATE), FIRST_ORDER_DATE, DAY) AS DAYS_SINCE_FIRST_ORDER,
  DIFF,
  CASE
    WHEN DIFF > 90
